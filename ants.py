...
if __name__ == "__main__":
    ...
    parser.add_argument('--listen', type=ip_address, metavar='address',
                        help="Master mode, listens for slave connections at.")
    ...
    try:
        if mode == MSLAVE:
            ...
            for i in range(0, (multiprocessing.cpu_count() - 2) if args.nestcount is 0 else args.nestcount):
                Nest(address=args.connect[0], port=args.port, loglevel=Nest.LOGLEVELS[args.loglevel],
            ...
            if mode == MMASTER:
                ...
                for i in range(0, (multiprocessing.cpu_count() - 2) if args.nestcount is 0 else args.nestcount):
                    Nest(address='127.0.0.1', port=args.port, loglevel=Nest.LOGLEVELS[args.loglevel],
                         name="%s_%d" % ('default', i))
